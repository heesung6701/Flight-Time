...
1. 운영자가 GitHub repository variable `AIRCRAFT_REGISTRATIONS`에 항공기 등록번호 목록을 저장합니다.
2. 매일 03:10 KST에 GitHub Actions 배치가 실행됩니다.
3. 배치는 variable의 등록번호를 정규화해 `data/aircraft-types.json`에 레코드로 추가합니다.
4. 배치는 `aircraftType`이 비어 있고 `lookup.status`가 `not_found`가 아닌 항목만 후보로 선택합니다.
5. 후보별로 AeroDataBox `GET /aircrafts/reg/{registration}` API를 호출합니다.
6. 성공한 항목은 기종, 모델, source metadata를 DB에 저장합니다.
7. DB 변경이 있으면 Action이 `data/aircraft-types.json` 변경분을 커밋하고 푸시합니다.

### 6.2 수동 등록번호 조회

1. 운영자가 GitHub Actions `workflow_dispatch`를 실행합니다.
2. `registrations` 입력에 쉼표 또는 공백으로 구분된 등록번호를 넣습니다.
3. 수동 입력값이 있으면 repository variable보다 우선 사용합니다.
4. 배치는 입력값을 정규화하고 DB에 레코드를 생성한 뒤 조회합니다.
5. 조회 결과는 정기 배치와 동일한 구조로 저장됩니다.
...
| FR-3 | GitHub repository variable `AIRCRAFT_REGISTRATIONS`로 정기 조회 대상 등록번호 목록을 주입할 수 있어야 한다. |
| FR-3a | `workflow_dispatch`의 `registrations` 입력값이 있으면 repository variable보다 우선 적용되어야 한다. |
...
| Repository variable | `AIRCRAFT_REGISTRATIONS` |
...
1. GitHub repository variable `AIRCRAFT_REGISTRATIONS`와 secret `AERODATABOX_API_KEY`를 설정합니다.
