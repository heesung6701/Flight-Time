# T'way Air 비즈니스 로직 문서

이 문서는 T'way Air의 `original` 입력과 `config` 입력을 어떤 방식으로 읽고, 최종 `output` 테이블의 각 컬럼을 어떻게 결정하는지 정리한 문서입니다.
...
웹사이트는 다음 두 방식으로 T'way `original` 데이터를 받습니다.
...
| DAY T/O | `dayTakeoff` | 주야간 분류 로직 결과 | original index 3, 7, 9, 14, 15, 16 |
| DAY L/D | `dayLanding` | 주야간 분류 로직 결과 | original index 3, 8, 9, 14, 15, 16 |
| NIGHT T/O | `nightTakeoff` | 주야간 분류 로직 결과 | original index 3, 7, 9, 14, 15, 16 |
| NIGHT L/D | `nightLanding` | 주야간 분류 로직 결과 | original index 3, 8, 9, 14, 15, 16 |
...
| `ro` | original index 7, 출발 시간 |
| `ri` | original index 8, 도착 시간 |
| `from` | original index 4, 출발 공항 IATA |
| `to` | original index 5, 도착 공항 IATA |
...
| **야간 착륙** | 도착 시간이 `To` 공항의 **야간** 범위에 속함 | 빈 값 | `landing` |

### 6.4 특수 Flight No의 Night 일부 포함 비행
...
`specialFlightNo(flightNo)`는 Flight No가 숫자로만 되어 있고, 첫 자리가 `0` 또는 `1`이며, 전체 숫자가 짝수이면 `true`입니다.
...
`원본`과 `출력` 개수가 다를 때 두 값의 툴팁에는 제외된 Duty별 건수와 최종 출력 건수를 표시합니다.
...
## 13. 주의사항 / 현재 T'way 비즈니스 규칙의 암묵적 가정
